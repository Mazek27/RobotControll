...
public class Search {
    ...
    public static String solution(String facelets, int maxDepth, long timeOut, boolean useSeparator) {
        ...
        try {
            ...
            return "Nie ma dokładnie jeden facelet każdego koloru";
        }
        for (int i = 0; i < 6; i++)
            if (count[i] != 9)
                return "Nie ma dokładnie jeden facelet każdego koloru";
        ...
        if ((s = cc.verify()) != 0){
            int code =  Math.abs(s);
            switch (code){
                case 2: return "Nie wszystkie 12 krawedzi istnieje dokładnie jeden raz ";
                case 3: return "Jedna krawedz powinna zostac przerzucona";
                case 4: return "Nie wszystkie rogi istnieja dokladnie raz";
                case 5: return "Jeden rog powinien zostac obrocony";
                case 6: return "Dwa rogi lub dwie krawędzie muszą być wymieniane";
                    ...
                    do {// increment axis
                        if (++ax[n] > 5) {
                            ...
                            if (System.currentTimeMillis() - tStart > timeOut << 10)
                                return "Nie ma rozwiazania w ciagu danego czasu";

                            if (n == 0) {
                                if (depthPhase1 >= maxDepth)
                                    return "Nie istnieje rozwiązanie dla danego max Ruchów";
                                else {
                                    depthPhase1++;
                                    ax[n] = 0;
                                    po[n] = 1;
